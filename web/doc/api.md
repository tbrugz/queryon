...
* `data-element` - element for data (default is `data`)
* `metadata` - returns (or not) the `$metadata` element (default is `true`)
* `table-as-data-element` - use table name as data element (default is `false`)
